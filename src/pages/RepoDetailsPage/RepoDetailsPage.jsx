...
const CodeBranch =lazy(()=> import('../../components/atoms/SVGRs/CodeBranch'));
const Watchers = lazy(()=> import('../../components/atoms/SVGRs/Watchers'));
const OpenIssues = lazy(()=> import('../../components/atoms/SVGRs/OpenIssues'))
...
const RepoDetailsPage = () => {
  ...
  return (
    <section className="repo-details">
        ...
        <BackIcon className="repo-details__back-icon" /> {labels.BACK_TO_REPOS}
      </Link>
      <h1 className="repo-details__title">{repo.name || labels.REPO_NAME}</h1>
      <p className="repo-details__description">{repo.description || labels.REPO_DESCRIPTION}</p>
      <p className="repo-details__language">{labels.REPO_LANGUAGE}: {repo.language || "N/A"}</p>
      <div className="repo-details__item">
          <CodeBranch /> <span>{labels.REPO_FORKS}:</span> {repo.forks}
        </div>

        <div className="repo-details__item">
          <OpenIssues /> <span>{labels.REPO_ISSUES}:</span> {repo.open_issues}
        </div>

        <div className="repo-details__item">
          <Watchers /> <span>{labels.REPO_WATCHERS}:</span> {repo.watchers}
        </div>
        <Button
